#include "scan_match_plicp/scan_match_plicp.h"
...
void ScanMatchPLICP::ScanCallback(const sensor_msgs::LaserScan::ConstPtr &scan_msg){
    ...
    ScanMatchWithLDPICP(curr_ldp_scan,scan_msg->header.stamp);
    end_time_=std::chrono::steady_clock::now();
    time_used_=std::chrono::duration_cast<std::chrono::duration<double>>(end_time_-start_time_);
    std::cout<<"PLICP计算用时: "<<time_used_.count()<<std::endl;
...
//使用PLICP进行计算
void ScanMatchPLICP::ScanMatchWithLDPICP(LDP &curr_ldp_scan,const ros::Time &time){
    ...
    if(output_.valid){
        std::cout<<"transform: ("<<output_.x[0]<<", "<<output_.x[1]<<", "
...
int main(int argc,char **argv){
    ros::init(argc,argv,"scan_match_plicp");
    ScanMatchPLICP scanmatchplicp;
