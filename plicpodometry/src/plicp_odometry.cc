...
#include "plicpodometry/plicp_odometry.h"
...
void ScanMatchPLICP::InitParams()
{
    ...
    last_velocity_.linear.x = 0;
    last_velocity_.angular.z = 0;
...
void ScanMatchPLICP::ScanMatchWithPLICP(LDP &curr_ldp_scan, const ros::Time &time)
{
    ...
    if (output_.valid)
    {
        ...
        current_velocity_.linear.x = corr_ch.getOrigin().getX() / dt;
        current_velocity_.angular.z = tf2::getYaw(corr_ch.getRotation()) / dt;
    }
    else
    {
                                   ...
                                   double dt)
{
    ...
    latest_velocity_.linear.x = (last_velocity_.linear.x + current_velocity_.linear.x) / 2;
    latest_velocity_.linear.y = (last_velocity_.linear.y + current_velocity_.linear.y) / 2;
    latest_velocity_.angular.z = (last_velocity_.angular.z + current_velocity_.angular.z) / 2;
    // 速度小于 1e-6 , 则认为是静止的
